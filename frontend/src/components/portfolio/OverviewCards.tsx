    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      {cards.map((card, idx) => (
        <motion.div
          ...
          className="glass-card p-6 rounded-2xl border border-white/5 bg-white/5 hover:bg-white/10 transition-all group"
        >
          ...
          <div className="flex flex-col">
            ...
            {card.label && !card.isText && (
              <div className="flex items-center mt-2 space-x-2">
                <span className={`text-sm font-semibold ${card.change >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                  {card.change >= 0 ? "+" : ""}{card.change.toFixed(2)} ({card.changePercent.toFixed(2)}%)
                </span>
          ...
          <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className={`w-2 h-2 rounded-full ${(!card.isText && card.change >= 0) || card.isText ? "bg-emerald-500/50" : "bg-rose-500/50"} blur-sm`} />
          </div>
        </motion.div>
      ))}
    </div>
